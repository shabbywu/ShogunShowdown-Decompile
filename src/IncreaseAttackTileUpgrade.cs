...
public class IncreaseAttackTileUpgrade : TileUpgrade
{
	...
	public override string Description
	{
		get
		{
			string text = string.Format("[reward_color_good]+{0} {1}[end_color]", attackDelta, TileUpgrade.LocalizedString("Damage"));
			if (cooldownDelta > 0)
			{
				text += string.Format("\n[reward_color_bad]+{0} {1}[end_color]", cooldownDelta, TileUpgrade.LocalizedString("Cooldown"));
			}
	...
	public override bool CanUpgradeTile(Tile tile)
	{
		...
		{
			return tile.Attack.Value < Attack.maxValue;
		}
		return false;
	}
	...
	public override string CannotUpgradeText(Tile tile)
	{
		...
		if (tile.Attack.Value == Attack.maxValue)
		{
			return base.CannotUpgradeHeader + TileUpgrade.LocalizedString("CannotUpgrade_MaxDamage");
		}
